locals {
  controller_nodes = [
    for i in range(var.controller_count) : {
      name = "ip-${replace(cidrhost(var.cluster_node_network, var.cluster_node_network_first_controller_hostnum + i), ".","-")}-cp${i}"
      address = cidrhost(var.cluster_node_network, var.cluster_node_network_first_controller_hostnum + i)
    }
  ]
  worker_nodes = [
    for i in range(var.worker_count) : {
      name    = "ip-${replace(cidrhost(var.cluster_node_network, var.cluster_node_network_first_worker_hostnum + i), ".","-")}-worker${i}"
